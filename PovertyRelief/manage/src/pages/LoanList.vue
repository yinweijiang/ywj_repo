  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
                       label="创建时间">
        <template slot-scope="aaa">
          <el-tag size="small">{{aaa.row.createTime}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="aaa">
          <el-button @click="deleteById(aaa.row.id)" size="mini" type="danger">删除</el-button>
          <router-link :to="{path:'/loanedit/'+aaa.row.id}">
  ...
  methods: {
    ...
    deleteById(id){
      this.$confirm('此操作将删除该公告, 是否继续?', '提示', {
        ...
        this.$axios("/student-loan/delete?id="+id).then(resp=>{
          this.$message({
            type:"warning",
    ...
    selectPage(current) {
      this.$axios.post("/student-loan/selectPage?current=" + current + "&size=8").then(resp => {
