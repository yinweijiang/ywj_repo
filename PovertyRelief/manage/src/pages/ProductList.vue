  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        label="发布用户id">
        <template slot-scope="aaa">
          <el-tag size="small">{{ aaa.row.releaseUserId }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="aaa">
          ...
          <router-link :to="{path:'/productedit/'+aaa.row.id}">
  ...
  methods: {
    ...
    deleteById(id) {
      this.$confirm('此操作将删除该商品, 是否继续?', '提示', {
        ...
        this.$axios("/product/delete?id=" + id).then(resp => {
          this.$message({
            type: "warning",
    ...
    selectPage(current) {
      this.$axios.post("/product/selectPage?current=" + current + "&size=8").then(resp => {
