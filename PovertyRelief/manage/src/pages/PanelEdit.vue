  ...
  <div>
    ...
    <el-form label-width="80px" size="small" label-position="right">
      <el-form-item label="标题" prop="title">
        <el-input auto-complete="off" v-model="panel.title"></el-input>
      </el-form-item>
      <el-form-item label="封面">
        <img style="padding-bottom:15px" v-if="panel.cover" :src="panel.cover" class="avatar" width="400"/>
        <br/>
        <input
          class="form-control"
          name="file"
...
export default {
  ...
  methods: {
    submit() {
      this.$axios.post("/panel/update", {
        id:this.panel.id,
        title:this.panel.title,
        cover: this.panel.cover,
    ...
    uploadCover(e) {
      ...
      this.$axios
        ...
        .then((response) => {
          this.panel.cover = response.data.data;
        });
    },
    getDate(){
      this.$axios.post("/panel/selectById?id="+this.$route.params.id).then(resp=>{
        this.panel = resp.data.data
        this.editor.txt.html(this.panel.detail);
      })
    }
  },
  mounted() {
