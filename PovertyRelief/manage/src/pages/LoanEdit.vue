  ...
  <div>
    ...
    <el-form label-width="80px" size="small" label-position="right">
      <el-form-item label="标题" prop="title">
        <el-input auto-complete="off" v-model="loan.title"></el-input>
      </el-form-item>
      <el-form-item label="封面">
        <img style="padding-bottom:15px" v-if="loan.cover" :src="loan.cover" class="avatar" width="400"/>
        <br/>
        <input
          class="form-control"
          name="file"
...
export default {
  ...
  methods: {
    submit() {
      this.$axios.post("/student-loan/update", {
        id:this.loan.id,
        title:this.loan.title,
        cover: this.loan.cover,
    ...
    uploadCover(e) {
      ...
      this.$axios
        ...
        .then((response) => {
          this.loan.cover = response.data.data;
        });
    },
    getDate(){
      this.$axios.post("/student-loan/selectById?id="+this.$route.params.id).then(resp=>{
        this.loan = resp.data.data
        this.editor.txt.html(this.loan.detail);
      })
    }
  },
  mounted() {
