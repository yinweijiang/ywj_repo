    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="aaa">
          <img :src="aaa.row.picture" style="width: 100%">
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    audit(id){
      this.$axios("/poverty-apply/audit?id=" + id).then(resp => {
        this.$message({
          type: "warning",
    ...
    deleteById(id) {
      this.$confirm('此操作将删除该请求,用户也可重新申请, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$axios("/poverty-apply/delete?id=" + id).then(resp => {
          this.$message({
            type: "warning",
            message: resp.data.data
          })
    ...
    selectPage(current) {
      this.$axios.post("/poverty-apply/selectPage?current=" + current + "&size=8").then(resp => {
