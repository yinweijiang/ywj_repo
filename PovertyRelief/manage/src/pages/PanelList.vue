    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template slot-scope="aaa">
          ...
          <router-link :to="{path:'/paneledit/'+aaa.row.id}">
  ...
  methods: {
    ...
    deleteById(id){
      this.$confirm('此操作将删除该公告, 是否继续?', '提示', {
        ...
        this.$axios.post("/panel/delete?id="+id).then(resp=>{
          this.$message({
            type:"warning",
    ...
    selectPage(current) {
      this.$axios.get("/panel/selectPage?current=" + current + "&size=8").then(resp => {
